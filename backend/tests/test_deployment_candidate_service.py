...
async def test_stage_candidate() -> None:
    ...
    actor_id = uuid4()
    returned = await DeploymentCandidateService(candidates, promotions).stage_candidate(DeploymentCandidateStageInput(item.id, actor_id, " staged "))
...
async def test_activate_retires_existing_active_in_same_flush() -> None:
    ...
    returned = await DeploymentCandidateService(candidates, promotions).activate_candidate(DeploymentCandidateActivateInput(item.id, uuid4()))
...
async def test_retire_and_reject_store_normalized_reasons() -> None:
    for method, dto, start, target in (("retire_candidate", DeploymentCandidateRetireInput, DeploymentCandidateStatus.ACTIVE, DeploymentCandidateStatus.RETIRED), ("reject_candidate", DeploymentCandidateRejectInput, DeploymentCandidateStatus.CANDIDATE, DeploymentCandidateStatus.REJECTED)):
        ...
        returned = await getattr(DeploymentCandidateService(candidates, promotions), method)(dto(item.id, uuid4(), "  reason  "))
...
async def test_invalid_transitions_and_consistency_do_not_mutate() -> None:
    ...
    with pytest.raises(DeploymentCandidateStateConflictError): await DeploymentCandidateService(candidates, promotions).stage_candidate(DeploymentCandidateStageInput(item.id, uuid4()))
    ...
    with pytest.raises(DeploymentCandidateConsistencyError): await DeploymentCandidateService(candidates, promotions).stage_candidate(DeploymentCandidateStageInput(item.id, uuid4()))
...
async def test_invalid_lifecycle_actor_is_rejected_before_repository_access(method, dto, actor_field, invalid_actor) -> None:
    candidates, promotions = repositories(); service = DeploymentCandidateService(candidates, promotions)
