...
def test_comparison_columns_constraints_indexes_and_foreign_keys() -> None:
    ...
    assert set(table.c) == {table.c.id, table.c.dataset_version_id, table.c.dataset_checksum, table.c.dataset_example_count, table.c.winner_experiment_id, table.c.ranking_metric, table.c.created_by_user_id, table.c.created_at, table.c.updated_at}
    ...
    expected_fks = {"dataset_version_id": ("dataset_versions.id", "fk_benchmark_comparisons_dataset_version_id"), "winner_experiment_id": ("benchmark_experiments.id", "fk_benchmark_comparisons_winner_experiment_id"), "created_by_user_id": ("users.id", "fk_benchmark_comparisons_created_by_user_id")}
    ...
    assert set(indexes(table)) == {"ix_benchmark_comparisons_dataset_version_id", "ix_benchmark_comparisons_winner_experiment_id", "ix_benchmark_comparisons_created_by_user_id", "ix_benchmark_comparisons_created_at"}
...
def test_member_columns_constraints_uniqueness_indexes_and_foreign_keys() -> None:
    ...
    assert set(table.c.keys()) == {"id", "benchmark_comparison_id", "benchmark_experiment_id", "rank", "created_at"}
    assert all(not column.nullable for column in table.c)
    ...
    assert set(expected) == {"uq_benchmark_comparison_members_comparison_experiment", "uq_benchmark_comparison_members_comparison_rank", "ix_benchmark_comparison_members_comparison_id", "ix_benchmark_comparison_members_experiment_id", "ix_benchmark_comparison_members_rank"}
    assert expected["uq_benchmark_comparison_members_comparison_experiment"].unique
    assert expected["uq_benchmark_comparison_members_comparison_rank"].unique
    comparison_fk = next(iter(table.c.benchmark_comparison_id.foreign_keys)); experiment_fk = next(iter(table.c.benchmark_experiment_id.foreign_keys))
    assert (comparison_fk.constraint.name, comparison_fk.ondelete) == ("fk_benchmark_comparison_members_comparison_id", "CASCADE")
    assert (experiment_fk.constraint.name, experiment_fk.ondelete) == ("fk_benchmark_comparison_members_experiment_id", "RESTRICT")
...
def test_relationships_are_bidirectional() -> None:
    ...
    assert BenchmarkComparison.winner_experiment.property.back_populates == "winning_comparisons"
    assert BenchmarkComparison.created_by_user.property.back_populates == "created_benchmark_comparisons"
    assert BenchmarkComparisonMember.benchmark_experiment.property.back_populates == "comparison_members"
    assert DatasetVersion.benchmark_comparisons.property.back_populates == "dataset_version"
    assert BenchmarkExperiment.comparison_members.property.back_populates == "benchmark_experiment"
...
def test_relationship_collections_work_in_memory() -> None:
    dataset, winner, creator = DatasetVersion(), BenchmarkExperiment(), User()
    comparison = BenchmarkComparison(dataset_version=dataset, winner_experiment=winner, created_by_user=creator)
    member = BenchmarkComparisonMember(comparison=comparison, benchmark_experiment=winner, rank=1)
...
def test_models_are_exported_and_migration_is_single_head() -> None:
    ...
    assert script.get_heads() == ["20260805_07"]
