...
class BenchmarkComparisonMember(UUIDPrimaryKeyMixin, Base):
    ...
    benchmark_experiment_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("benchmark_experiments.id", name="fk_benchmark_comparison_members_experiment_id", ondelete="RESTRICT"), nullable=False)
    rank: Mapped[int] = mapped_column(Integer, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)

    __table_args__ = (
        CheckConstraint("rank >= 1", name="ck_benchmark_comparison_members_rank_positive"),
        Index("uq_benchmark_comparison_members_comparison_experiment", "benchmark_comparison_id", "benchmark_experiment_id", unique=True),
        ...
        Index("ix_benchmark_comparison_members_experiment_id", "benchmark_experiment_id"),
        Index("ix_benchmark_comparison_members_rank", "rank"),
    )

    comparison: Mapped[BenchmarkComparison] = relationship(back_populates="members")
    benchmark_experiment: Mapped[BenchmarkExperiment] = relationship(back_populates="comparison_members")
